...
const update = async (q, r) => {// #swagger.tags = ['Comments']
    try {
        const { headers: { headers_user_id }, body: { post_id, user_id, coment_content }, params: { id } } = q;
        if (headers_user_id !== user_id) {
            throw { message: 'No puedes editar el comentario de otra persona', status: 401 };
        };
